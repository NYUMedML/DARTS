...
rest_available = [4,   5,   7,   8,  10,  11,  12,  13,  14,  15,
        16,  17,  18,  24,  26,  28,  30,  31,  43,  44,  46,
        47,  49,  50,  51,  52,  53,  54,  58,  60,  62,  63,  72,  77,
        80,  85, 251, 252, 253, 254, 255]
...
class BrainImages(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        target_label = np.zeros((42,256,256))
        for i,a in enumerate(available_segments):
            ...
            if a in rest_available:
                ...
                target_label[41,:,:] = target_label[41,:,:] + temp
        target_label[41,:,:] = (target_label[41,:,:]>=1).astype(int) 
...
df_seg = pd.DataFrame(columns=np.arange(42))
ll = 0
batch = 30
...
#df_seg.to_csv("two_d_segment_count_job",index=False)
...
class Unet(nn.Module):
    def __init__(self):
        ...
        self.outconv = nn.Conv2d(64,42, 1)
...
def train_model(model, optimizer,dataloader, data_sizes, batch_size, num_epochs = 100, verbose = False):
    ...
    dice_scores_of_all_class = [{'train':[],'validate':[]} for i in range(42)]    
    for i in range(num_epochs):
        for phase in ['train', 'validate']:
            running_loss = 0
            run_class_scores = [0]*42
            if phase == 'train':
                model.train(True)
            else:
                model.train(False)
...
#pickling(model,"semi_model_rest_job1")
pickling(dice_hist,"semi_model_rest2_dice_job")
pickling(loss_hist,"semi_model_rest2_loss_job")

torch.save(model.state_dict(), "semi_model_rest_job2")
